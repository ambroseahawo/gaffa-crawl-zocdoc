...
from src.config.constants import TIME_LIMIT_MS, ZOCDOC_ORIGIN
...
async def browse_capture_envelope(
    ...
    *,
    ...
    async with gaffa_sem:
        ...
        _, debug_envelope = await run_browser_request_to_completion(
            session, api_key, body, options=options
        )
...
async def crawl_profile_indexes(api_key: str) -> None:
    ...
    logger.info(
        "crawl start seed=%s index_batch=%d gaffa_slots=%d queue=%d",
        target_url,
        index_batch,
        sem_slots,
        len(state.queue),
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=600)) as session:
        ...
        while state.queue:
            ...
            if provider_tasks:
                logger.info("cycle=%d provider jobs=%d", cycle, len(provider_tasks))
